...
INSERT INTO exam_score VALUES('박하나', 118, 62, 75);
...
-- NTITLE(수) OVER(ORDER BY 칼럼명)
-- 주어진 수만큼 행들을 n등분한 후
-- 현재 행에 해당하는 등급을 구하는 함수
-- 국어 과목 등급 검색
SELECT 이름,
       NTITE(8) OVER(ORDER BY 국어 DESC) 국어등급,
       국어,
       NTITE(8) OVER(ORDER BY 영어 DESC) 국어등급,
       영어,
       NTITE(8) OVER(ORDER BY 수학 DESC) 국어등급,
       수학
FROM exam_score
WHERE 이름 <> '윤하나'; -- '<>'는 '='의 반대
